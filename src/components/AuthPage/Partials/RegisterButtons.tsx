...
const RegisterButtons: React.FC = (): JSX.Element => {
  const { handleRegisterOAuth, loading } = useRegister();
    ...
    <>
      {loginProviders.map((provider) => (
        <button
          ...
          onClick={() => handleRegisterOAuth(provider.provider)}
          ...
          disabled={loading[provider.provider as 'google' | 'github']}
        >
          {loading[provider.provider as 'google' | 'github'] ? (
            <Spinner />
          ) : (
            <>
              <img
